...
<template>
  ...
  >
    <section class="max-w-2xl space-y-5">
      ...
      <p class="max-w-xl text-xl leading-8 text-[#b7c8dc]">Local Usage Metadata will appear here after the first Usage Sync.</p>
    </section>
    ...
    >
      <span v-if="status?.ok" class="inline-flex rounded-full bg-[#78f0aa] px-3 py-1.5 font-bold text-[#062113]">
        Connected
      </span>
      <span v-else-if="error" class="inline-flex rounded-full bg-[#ff8f8f] px-3 py-1.5 font-bold text-[#2c0707]">
        Disconnected
      </span>
      <span v-else class="inline-flex rounded-full bg-[#bdd1e8] px-3 py-1.5 font-bold text-[#0d1724]">
        Connecting
      </span>
      <p class="pt-2 text-xs font-bold tracking-[0.12em] text-[#8fb5e3] uppercase">Analytics Store</p>
